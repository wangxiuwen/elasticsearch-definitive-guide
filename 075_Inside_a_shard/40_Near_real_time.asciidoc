...
默认情况下每个分片会每秒自动刷新一次。这就是为什么我们说 Elasticsearch 有 _准_ 实时搜索: 
...
你的应用应该需要意识到 Elasticsearch 的准实时的性质， 并且容忍它。
====

并不是所有的情况都需要每秒刷新。可能你正在使用 Elasticsearch 索引大量的日志文件， 你可能想优化索引速度而不是准实时搜索，
